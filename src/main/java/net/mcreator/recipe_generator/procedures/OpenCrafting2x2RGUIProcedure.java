...
public class OpenCrafting2x2RGUIProcedure {
		...
		if ((RecipeGeneratorModVariables.WorldVariables.get(world).selectedMethod).equals("NONE")) {
			...
			if (entity instanceof ServerPlayer _ent) {
				...
				_ent.openMenu(new MenuProvider() {
					@Override
					public Component getDisplayName() {
						return Component.literal("Crafting2x2RGUI");
					}
					...
					@Override
					public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
						return new Crafting2x2RGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
					}
				}, _bpos);
			}
		}
