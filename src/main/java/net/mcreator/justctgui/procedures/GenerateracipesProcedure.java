...
public class GenerateracipesProcedure {
	public static void execute(Entity entity, HashMap guistate) {
		...
		if ((guistate.containsKey("text:recipe_name") ? ((EditBox) guistate.get("text:recipe_name")).getValue() : "").isEmpty() && !JustCtguiModVariables.Is_shapeless && !JustCtguiModVariables.Is_mirrored) {
			...
			JustCtguiModVariables.Generated_recipe = "import crafttweaker.api.recipe.MirrorAxis;" + "craftingTable.addShapedMirrored(\"" + "no_name" + "\", " + "MirrorAxis." + JustCtguiModVariables.Mirror_axis + ", "
					+ JustCtguiModVariables.Pre_generated_recipe;
		...
		if (entity instanceof Player _player && !_player.level().isClientSide())
			_player.displayClientMessage(Component.literal(JustCtguiModVariables.Generated_recipe), false);
	}
}
