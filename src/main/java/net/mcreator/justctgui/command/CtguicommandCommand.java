...
public class CtguicommandCommand {
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("ctgui").requires(s -> s.hasPermission(4)).then(Commands.literal("crafting_table").then(Commands.literal("open").executes(arguments -> {
			...
			return 0;
		}))).then(Commands.literal("furnace").then(Commands.literal("open").executes(arguments -> {
			ServerLevel world = arguments.getSource().getLevel();
			double x = arguments.getSource().getPosition().x();
			double y = arguments.getSource().getPosition().y();
			double z = arguments.getSource().getPosition().z();
